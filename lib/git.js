 ...
 * @param options.annotated whether to create an annotated tag or soft tag (default: true)
 * @param cb callback function to execute
 */
var add_tag_object  = function(options, cb) {
  options.annotated = options.annotated || true;
  var type = options.annotated ? '-a' : '';
  exec(util.format("git tag %s %s -m '%s' %s", type, options.tag_name, options.tag_message, options.hash), 
   {cwd: options.cwd}, 
 ...
 * @param options.tag_name tag name
 * @param options.tag_message tag message
 * @param options.overwrite whether or not to overwrite existing tag (default: false)
 * @param options.annotated whether to create an annotated tag or soft tag (default: true)
 * @param cb callback function to execute
 */
exports.add_tag = function(options, cb) {
  ...
  add_tag_object(options, function(error, stdout) {
    // If overwrite is true & tag already exists, delete it and tag the new object.
    if (options.overwrite && error && error.hasOwnProperty('error') 
